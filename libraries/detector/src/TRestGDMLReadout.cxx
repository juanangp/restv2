...
void TRestGDMLReadout::BuildGeometry() {
    if (!fNode || !fReadoutNode) {
        throw std::runtime_error(
            "TRestGDLMReadout: 'readoutParameters' section is missing or fNode is not initialized!");
    }
    ...
    if (!fGeoManager) {
        throw std::runtime_error("TRestGDMLReadout::BuildGeometry - ROOT failed to parse GDML input: " +
                                 fGDMLFileName);
    }
