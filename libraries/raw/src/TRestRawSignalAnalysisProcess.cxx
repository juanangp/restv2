...
static const bool TRestRawSignalAnalysisProcess_FieldsRegistered = []() {
    ...
    reg.RegisterField<TRestRawSignalAnalysisProcess>("baselineRange",
                                                     &TRestRawSignalAnalysisProcess::fBaselineRange);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("integralRange",
                                                     &TRestRawSignalAnalysisProcess::fIntegralRange);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("baselineOption",
                                                     &TRestRawSignalAnalysisProcess::fBaselineOption);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("pointsThreshold",
                                                     &TRestRawSignalAnalysisProcess::fPointsThreshold);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("signalThreshold",
                                                     &TRestRawSignalAnalysisProcess::fSignalThreshold);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("pointsOverThreshold",
                                                     &TRestRawSignalAnalysisProcess::fPointsOverThreshold);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("signalsRange",
                                                     &TRestRawSignalAnalysisProcess::fSignalsRange);
    reg.RegisterField<TRestRawSignalAnalysisProcess>("nPointsFlat",
                                                     &TRestRawSignalAnalysisProcess::fNPointsFlat);
...
const bool kRegistered = []() {
    MetadataClassRegistry::Instance().Register(
        "TRestRawSignalAnalysisProcess", [](const std::string& instanceName, const YAML::Node& params) {
...
bool TRestRawSignalAnalysisProcess::ProcessEvent(const TRestEvent& input, TRestEvent& output) {
    ...
    for (int s = 0; s < sigEvent->GetNumberOfSignals(); s++) {
        ...
        if (fRangeEnabled && (sID < fSignalsRange.first || sID > fSignalsRange.second)) continue;

        double baseline, baselineSigma;
        auto data = CalculateBaselineAndSigma(signal.GetData(), fBaselineRange.first, fBaselineRange.second,
                                              baseline, baselineSigma, fBaselineOption);

        auto pointsOverThr = GetPointsOverThreshold(data, fIntegralRange, thrParams, fPointsOverThreshold,
                                                    fNPointsFlat, baselineSigma);
        ...
        std::pair<double, double> minVal{std::numeric_limits<double>::max(),
    ...
    fObs.BaselineSigmaAvg =
        std::accumulate(fObs.BaselineSigma.begin(), fObs.BaselineSigma.end(), 0.0) / fObs.NSignals;

    auto calcAvgOverThr = [](const auto& data, const std::vector<int>& pointsThr,
    ...
    fObs.IntegralBalance = (fObs.FullIntegralSum - fObs.ThresholdIntegralSum) /
                           (fObs.FullIntegralSum + fObs.ThresholdIntegralSum);
